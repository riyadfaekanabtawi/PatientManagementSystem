...
# Fix for deprecated inspect.getargspec (if needed)
def fixed_depends_on(func):
    if "getargspec" in dir(inspect):
        return "out" in inspect.getargspec(func).args
    else:
        return "out" in inspect.getfullargspec(func).args

# -------------------------------------------------------------------
# Initialize DECA in a try/except block so that any errors during
# model loading are logged and the service exits gracefully.
try:
